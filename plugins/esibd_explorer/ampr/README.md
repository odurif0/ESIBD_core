# AMPR Plugin For ESIBD Explorer
...
2. Set the Explorer `plugin path` to the directory that contains the `ampr/`
   folder.
...
4. Enable the `AMPR` plugin in the Plugin Manager.

The plugin lazily loads its bundled local
`vendor/esibd_ampr_plugin_runtime` runtime when communication is initialized.
...
To copy this plugin to another machine, keep the whole `ampr/` directory
