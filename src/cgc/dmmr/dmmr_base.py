...
class DMMRBase:
    ...
    def get_base_state(self):
        ...
        active_states = []
        if state_value == 0:
            active_states.append('BASE_OK')
        else:
            for flag, name in self.BASE_STATE.items():
                if state_value & flag:
                    active_states.append(name)
    ...
    def get_base_fan_pwm(self):
        ...
        return status, set_pwm.value, hex(state_value), active_states
    ...
    def set_current_config(self, config_data):
        ...
        """
        config = (ctypes.c_uint32 * self.MAX_REG)(*config_data[:self.MAX_REG])
